...
public class BoardController {
	...
	@RequestMapping("list.do")	// 전체 목록, 검색 목록 //board에 list.jsp에서 전달된 keyword와 search값이 넘어옴 
	public String list(String pageNum, Board board, Model model) {
		final int rowPerPage = 10;	// 화면에 출력할 데이터 갯수 //final은 상수가 되서 수정 불가능
		if (pageNum == null || pageNum.equals("")) { //페이지값이 없으면 무조건 1페이지로 가겠다는 것.
			pageNum = "1";
		}
		int currentPage = Integer.parseInt(pageNum); // 현재 페이지 번호. 페이지값이 있다면 현재 페이지 값으로 간다는 의미
		
		// int total = bs.getTotal();
		int total = bs.getTotal(board); // 검색 (총 데이터 갯수 저장). 검색을 했을 경우와 일반적인  총 데이터도 저장함. 이것은 나눠서 저장되는데 동적SQL문으로 결정됨
	...
	public String insert(Board board, Model model, HttpServletRequest request) {
		...
		} else				// 원문 num값이 0으로 전달 받은 것	
			board.setRef(number); //else끝
	...
	@RequestMapping("deleteForm.do") // 삭제폼
